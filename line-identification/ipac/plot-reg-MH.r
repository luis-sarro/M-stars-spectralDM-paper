...
c <- SetupPalette(teff)
lims <-  rbind(
               c(8516.4,0,8538.0,0.035),
               c(8620.8,0,8628.0,0.035),
               c(8782.8,0,8790.0,0.035),
               c(8970.0,0,8977.2,0.035),
               c(9009.6,0,9016.8,0.035))

lims2 <-  rbind(
               c(8548.8,0,8577.6,0.035),
               c(8635.2,0,8671.2,0.035),
               c(8797.2,0,8818.8,0.035),
               c(8916.0,0,8952.0,0.035),
               c(8980.8,0,9002.4,0.035))

               
for (j in 1:5){
  jpeg(paste("Feature-MH",j,".jpg",sep=""),quality=100,width=1000, height=1000)
  par(mar=c(7,7,5,10))
  ...
  jpeg(paste("All-MH.jpg",sep=""),quality=100,width=1000, height=1000)
  par(mar=c(7,7,5,10))
  plot(wav,data[1,], ty="l",col=c[1],ylim=c(0,0.035),main=paste("Feature ",j,sep=""),
       cex.axis=2, cex.lab=2, xlab="Lambda(Angstroms)", ylab="Flux(relative units)")
