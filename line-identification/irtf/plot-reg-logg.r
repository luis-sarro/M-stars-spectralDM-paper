...
c <- SetupPalette(teff)
lims <-  rbind(
               c(8462.4,0,8476.8,0.035),
               c(8620.8,0,8628.0,0.035),
               c(8646.0,0,8653.2,0.035),
               c(8656.8,0,8664.0,0.035),
               c(8782.8,0,8790.0,0.035))

lims2 <-  rbind(
               c(8484.0,0,8520.0,0.035),
               c(8667.6,0,8703.6,0.035),
               c(8689.2,0,8710.8,0.035),
               c(8689.2,0,8718.0,0.035),
               c(8797.2,0,8818.8,0.035))

               
for (j in 1:5){
  jpeg(paste("Feature-logg",j,".jpg",sep=""),quality=100,width=1000, height=1000)
  par(mar=c(7,7,5,10))
  ...
  jpeg(paste("All-logg.jpg",sep=""),quality=100,width=1000, height=1000)
  par(mar=c(7,7,5,10))
  plot(wav,data[1,], ty="l",col=c[1],ylim=c(0,0.035),main=paste("Feature ",j,sep=""),
       cex.axis=2, cex.lab=2, xlab="Lambda(Angstroms)", ylab="Flux(relative units)")
